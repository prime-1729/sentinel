...
def health():
    ...
    return {
        "status": "operational",
        "live_monitor_running": sidecar.is_running() if sidecar else False,
...
async def analyze_mission(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Generate a unique mission ID for the uploaded log
        mission_id = f"mission_{uuid.uuid4().hex[:8]}"
        report = generate_intelligence_report(telemetry, anomalies, mission_id=mission_id)
...
def telemetry_live():
    ...
    return {
        ...
        "monitor_running": sidecar.is_running() if sidecar else False,
...
def monitor_start(
    ...
    if sidecar:
        sidecar.start(connection_string=connection)
...
# Initialize the new ReAct Agent
agent = ReActAgent(model="qwen3")

@app.post("/ask")
def ask_agent(req: AskRequest):
    """Process a natural language question via the SENTINEL agent."""
    answer = agent.ask(req.question, mission_id=req.mission_id, session_id=req.session_id)
