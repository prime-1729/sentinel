...
class TelemetryStore:
    ...
    def clear_mission_data(self, mission_id: str) -> None:
        """
        Clears all telemetry and anomaly data for a given mission.
        Useful for resetting live monitoring sessions.
        """
        with self.conn:
            tables = ['positions', 'battery', 'attitude', 'hud', 'anomaly_events']
            for table in tables:
                self.conn.execute(f"DELETE FROM {table} WHERE mission_id = ?", (mission_id,))
            self.conn.execute("DELETE FROM missions WHERE mission_id = ?", (mission_id,))
